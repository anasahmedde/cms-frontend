...
// MERGED: Original features + New Announcement System + Improved Notifications
...
function AnnouncementModal({ isOpen, onClose, onPublish, currentAnnouncement, publishing }) {
  ...
  const typeColors = {
    info: { bg: "linear-gradient(90deg, #3b82f6, #1d4ed8)", text: "#fff", label: "ℹ️ Info (Blue)" },
    warning: { bg: "linear-gradient(90deg, #f59e0b, #d97706)", text: "#0a1628", label: "⚠️ Warning (Yellow)" },
    critical: { bg: "linear-gradient(90deg, #dc2626, #b91c1c)", text: "#fff", label: "🚨 Critical (Red)" },
  };
  ...
  return (
    <>
      ...
      <div style={{
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {Object.entries(typeColors).map(([key, val]) => (
              <button key={key} onClick={() => setType(key)} style={{
                flex: 1, padding: "10px 12px", borderRadius: 8, border: type === key ? "2px solid #0f172a" : "1px solid #e5e7eb",
                background: type === key ? "#f8fafc" : "#fff", cursor: "pointer", fontSize: 12, fontWeight: 600,
        ...
        {message.trim() && (
          <div style={{ marginBottom: 20 }}>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 8 }}>Preview</label>
            <div style={{
              background: typeColors[type].bg, padding: "12px 20px", borderRadius: 10,
...
function SlidingAnnouncementBanner({ announcement, onClear }) {
    ...
    <div style={{
      ...
      {onClear && (
        <button onClick={onClear} style={{
          ...
          padding: "4px 10px", borderRadius: 6, cursor: "pointer", marginLeft: 12, fontSize: 12, fontWeight: 600
        }}>Clear</button>
      )}
      <style>{`
        @keyframes slideText {
...
// ─── Notification Bell Component (IMPROVED) ───
...
export default function PlatformDashboard() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      {/* ─── Custom Announcement Banner (NEW) ─── */}
      {announcement && (
        <SlidingAnnouncementBanner announcement={announcement} onClear={clearAnnouncement} />
      )}

      {/* ─── Critical Alert Static Banner (when companies expired) ─── */}
      {expiredCount > 0 && (
        <div style={{
          background: "#dc2626",
          padding: "14px 20px",
      ...
      {/* ─── Header with Notification Bell + Announce Button ─── */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button onClick={() => setShowAnnouncementModal(true)} style={{
            padding: "8px 16px", background: "#f8fafc", border: "1px solid #e5e7eb",
